...
from backend_sender import send_frame_to_backend, set_backend_url
...
_EAR_CLOSURE_THRESHOLD = 0.18


def main() -> None:
    cfg = CFG

    PATIENT_ID = select_patient("FACE/EYE MONITORING")

    # ── Backend server ──────────────────────────────────────────────────
    try:
        ip = input("Enter backend server IP (default localhost): ").strip()
    ...
    print(f"[INFO] Camera {actual_w}x{actual_h} @ {cfg.camera.target_fps} fps")
    ...
    logger      = DistressLogger(subject_id=PATIENT_ID)
        ...
        if results.multi_face_landmarks:
            ...
            if patient_box is not None:
                ...
                if face_crop.size > 0:
                    ...
                    gaze_data = gaze_tracker.update(gaze_yaw, gaze_pitch, eye_closed)
