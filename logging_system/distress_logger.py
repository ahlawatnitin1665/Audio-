...
class DistressLogger:
    def __init__(self, subject_id="P001") -> None:
        cfg = CFG.logging
    ...
    def log_frame(
        ...
        active_types = {ep.episode_type.name for ep in active_episodes}
        ...
        row = {
            ...
            "gaze_variance": round(gaze_data.get("gaze_variance", 0), 4) if gaze_data else 0,
            "fixation_duration": round(gaze_data.get("fixation_duration", 0), 4) if gaze_data else 0,
            "recent_saccades": gaze_data.get("recent_saccades", 0) if gaze_data else 0,
            "mean_gaze_speed": round(gaze_data.get("mean_gaze_speed", 0), 4) if gaze_data else 0,
            "max_gaze_speed": round(gaze_data.get("max_gaze_speed", 0), 4) if gaze_data else 0,
            "eye_contact_ratio": round(gaze_data.get("eye_contact_ratio", 0), 4) if gaze_data else 0,
            ...
            "mar": round(au.mar, 4),
            "episode_pain": "PAIN" in active_types,
            "episode_agitation": "AGITATION" in active_types,
            "episode_fatigue": "FATIGUE" in active_types,
            "episode_fear": "FEAR" in active_types,
            "episode_eye_closure": "EYE_CLOSURE" in active_types,
